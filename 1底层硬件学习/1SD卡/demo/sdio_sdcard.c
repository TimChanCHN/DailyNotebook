...
{	  
	...
	if(CardType==SDIO_HIGH_CAPACITY_SD_CARD)//大容量卡
	{
		blksize=512;
		// TODO: 为什么地址要右移9位？
		// 因为传入的addr先左移了9位，并且块大小是512kb(2^9)，因此该addr是SD卡内地址
		// 以块大小作为标准
		addr>>=9;
	}   
	// 设置DCTRL寄存器，设置数据传输方向，并开始数据传输
	...
	// (blksize&(blksize-1)):保证块大小4字节对齐
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
	{
		//发送CMD16+设置数据长度为blksize,短响应
		power=convert_from_bytes_to_power_of_two(blksize);			// power可用于设置block size大小
...
SD_Error SD_WriteMultiBlocks(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	...
	{					  
		...
		{
			//提高性能
			SDIO_CmdInitStructure.SDIO_Argument = (u32)RCA<<16;		//发送CMD55,短响应 	
			SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
			SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
			SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
